...
/*id semaforo code di messaggi dei nodi*/
...
int main(int argc, char const *argv[])
{
    ...
    if (errno == EINVAL)
    {
        perror("shmat puntatoreSharedMemoryAmiciNodi");
        exit(EXIT_FAILURE);
    }
                 ...
                 /*   operazioniSemaforo.sem_op = -1;
                    ...
                    {   
                        ...
                        if(messaggioInviato.hops == 0){
                            /*msgsndRisposta = msgsnd(libro Mastro, &messaggioInviato, sizeof(messaggioInviato.transazione) + sizeof(messaggioInviato.hops), 0);*/
                           /* errno = 0;
                            printf("----------------soHops == 0-------------\n");
                        }
