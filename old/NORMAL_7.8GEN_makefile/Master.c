...
void stampaTerminale(int flag)
{
    ...
    int posizioneSalvata = *(puntatoreSharedMemoryIndiceLibroMastro);
    ...
    if (getSoUsersNum() < 2)
    {
        printf("\nUTENTE[PID] | BILANCIO[INT] | STATO[INT]\n");
        for (contatoreStampa = 0; contatoreStampa < getSoUsersNum(); contatoreStampa++){
            ...
            printf("%09d\t%09d\t%09d\n", printUtenti[contatoreStampa].userPid, printUtenti[contatoreStampa].budget, printUtenti[contatoreStampa].stato);
        }     
    }
    else
    {
