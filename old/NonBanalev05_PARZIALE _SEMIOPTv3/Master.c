...
int main(int argc, char const *argv[])
{
    ...
    idSharedMemoryAmiciNodi = shmget(IPC_PRIVATE, getSoNodesNum() * getSoFriendsNum() * sizeof(int), 0600 | IPC_CREAT);
    ...
    arrayValoriInizialiSemaforiCodeMessaggi = (unsigned short *)calloc(getSoNodesNum(), sizeof(unsigned short));
...
#endif
        for (j = 0; j < getSoFriendsNum(); j++)
        {
            if (nextFriend == getSoNodesNum())
            {
                nextFriend = 0;
            }
            puntatoreSharedMemoryAmiciNodi[(i - 1) * getSoFriendsNum() + j] = puntatoreSharedMemoryTuttiNodi[nextFriend + 1].nodoPid;
#if (ENABLE_TEST)
    ...
    while (master)
    {
        ...
        {
            /*creo un nuovo nodo, gli asssegno amici e gli mando la transazione*/
        }
    }
    ...
    eseguiDetachShm(puntatoreSharedMemoryAmiciNodi);
    ...
    eseguiDeallocamentoShm(idSharedMemoryAmiciNodi);
    ...
    eseguiDetachShm(puntatoreSharedMemoryTuttiUtenti);
    ...
    eseguiDeallocamentoShm(idSharedMemoryTuttiUtenti);
    ...
    eseguiDetachShm(puntatoreSharedMemoryTuttiNodi);
    ...
    eseguiDeallocamentoShm(idSharedMemoryTuttiNodi);
    ...
    eseguiDetachShm(puntatoreSharedMemoryIndiceLibroMastro);
    ...
    eseguiDeallocamentoShm(idSharedMemoryIndiceLibroMastro);
    ...
    eseguiDetachShm(puntatoreSharedMemoryLibroMastro);
    ...
    eseguiDeallocamentoShm(idSharedMemoryLibroMastro);
