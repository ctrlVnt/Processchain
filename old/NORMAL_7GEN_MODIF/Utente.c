...
/*TEST*/
    ...
    {
        printf("%d fa EXIT CON STATUS %d e alarm %d\n", getpid(), user, alrm);
        exit(EXIT_SUCCESS);
    }
}
...
int inviaTransazione(int flag)
{
    ...
    if (t.receiver != -1)
    {
        if (puntatoreSharedMemoryTuttiUtenti[numeroOrdine + 1].budget >= 2)
        {
            ...
            q = /*getQuantitaRandom(puntatoreSharedMemoryTuttiUtenti[numeroOrdine + 1].budget)*/ 2;
            /*printf("%d\n", puntatoreSharedMemoryTuttiUtenti[numeroOrdine + 1].budget);*/
            // printf("Quantita' estratta: %d\n", q);
            t.reward = /*(q * getSoReward()) / 100*/ 1;
            if (t.reward == 0)
            {
                t.reward = 1;
            }
