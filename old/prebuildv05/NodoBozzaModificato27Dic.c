...
int main(int argc, char const *argv[])
{
    ...
    while (node)
    {
        while (tpPiena < SO_TP_SIZE && node == NODO_CONTINUE && riempimentoTpStop)
        {
            msgrcvRisposta = msgrcv(puntatoreSharedMemoryTuttiNodi[numeroOrdine + 1].mqId, &messaggioRicevuto, sizeof(messaggioRicevuto.transazione), 0, IPC_NOWAIT);
            /*TEST*/
            ...
            if(errno != EAGAIN && msgrcvRisposta != -1){
                ...
                errno = 0;
                if(bloccoRiempibile == SO_BLOCK_SIZE - 1){
                    riempimentoTpStop = 0;
                }else{
                    attesaNonAttiva(1000,2000);
                }
            }
        ...
        if(node == NODO_STOP)
        {
            /*bisogna riprodurre il ciclo di vita del nodo con relativo reserve del semaforo per scrivere sul libro mastro*/
            /*introduco svuotamento transaction pool
            while(tpPiena > SO_BLOCK_SIZE - 1){
