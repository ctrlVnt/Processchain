...
int main(int argc, char const *argv[])
{
    ...
    soHops = (int)strtol(/*PRIMO PARAMETRO DELLA LISTA EXECVE*/ argv[7], /*PUNTATTORE DI FINE*/ &endptr, /*BASE*/ 10);
    if (soHops == 0 && errno == EINVAL)
    {
        perror("Errore di conversione");
        exit(EXIT_FAILURE);
    }
    ...
    #endif
    soRegistrySize = (int)strtol(/*PRIMO PARAMETRO DELLA LISTA EXECVE*/ argv[12], /*PUNTATTORE DI FINE*/ &endptr, /*BASE*/ 10);
    if (soRegistrySize == 0 && errno == EINVAL)
    {
        perror("Errore di conversione soRegistrySize");
        exit(EXIT_FAILURE);
    }
    #if(ENABLE_TEST)
        printf("N soRegistrySize: %d\n", soRegistrySize);
    #endif
    soNumFriends= (int)strtol(/*PRIMO PARAMETRO DELLA LISTA EXECVE*/ argv[13], /*PUNTATTORE DI FINE*/ &endptr, /*BASE*/ 10);
    if (soNumFriends == 0 && errno == EINVAL)
    {
        perror("Errore di conversione soNumFriends");
        exit(EXIT_FAILURE);
    }
    #if(ENABLE_TEST)
    ...
    while (node)
    {
        ...
        if (indiceLibroMastroRiservato >= soRegistrySize) /*11 parametro settato*/
        {
            node = NODO_STOP;
            break;
        }
