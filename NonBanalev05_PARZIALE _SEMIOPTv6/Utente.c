        ...
        if (puntatoreSharedMemoryTuttiUtenti[numeroOrdine + 1].budget >= 2)
        {
            ...
            if (semReserve(idSemaforoAccessoCodeMessaggi, -1, (idNodo - 1), IPC_NOWAIT) == -1 && errno == EAGAIN)
            {
                ...
                do
                {
                    ...
                    /*idNodo = puntatoreSharedMemoryTuttiNodi[puntatoreSharedMemoryAmiciNodi[(idNodo - 1) * (getSoFriendsNum() * 2 + 1) + idAmico]].;*/
                    // printf("Ho scelto %d che avrebbe il PID = %d\n\n", idNodo, puntatoreSharedMemoryTuttiNodi[idNodo].nodoPid);
                    if (semReserve(idSemaforoAccessoCodeMessaggi, -1, (idNodo - 1), IPC_NOWAIT) == -1 && errno == EAGAIN)
                    {
                        m.hops--;
                    }
...
void deallocaRisorse()
{
    ...
    eseguiDetachShm(puntatoreSharedMemoryTuttiUtenti, "puntatoreSharedMemoryTuttiUtenti");
